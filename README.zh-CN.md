...
实机运行前，请先阅读 [SAFETY.md](SAFETY.md)。
...
- 支持原始轨迹录制与回放。
- 支持 RealSense 双相机录制；当前随项目提供并测试的配置使用 Intel RealSense D455 和 L515，但项目并不限于这两款型号。
...
- Robotiq 夹爪，支持 socket 控制。
- 可被 Linux input 设备读取的 Xbox 兼容手柄。
- Intel RealSense RGB-D 相机。当前随项目提供并测试的配置使用 D455 和 L515。
...
- Xbox 手柄、已配置的 RealSense 相机和机器人网线已连接。
...
如果你在学术工作中使用 JoyCapture-UR5，建议通过 [CITATION.cff](CITATION.cff) 中的元数据进行引用。
